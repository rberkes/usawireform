...
- Source (/source) is the shop landing: we send a buyer job only to shops that filed a machine that can run it (wire size, 2D or 3D, make, city) — not every shop that says they form wire. Shops claim a US directory listing or file one cell free. Instant estimate stays this floor. Shop dashboard /source/dashboard. Account /source/account (email, password). Plans /source/upgrade: one cell free; $30/mo for 4; $49/mo for 10; $99/mo for 20. Secondaries $5/mo each from the dashboard (end forming, weld, plate, heat treat, inspection). Newest members: /directory/new. Confirm account from the equipment receipt or the on-page button. A Source shop public page is /directory/[slug]. Existing US directory shops claim that same URL then file cells. Canada stays in the directory only. Europe later, on its own platform.
